...
import { submitLead } from '@/src/lib/contact'
      ...
      <div className="page-body">
            ...
            <div className="aside-stack">
              <motion.div className="card aside-card" {...rev}>
                ...
                <ul className="arrow-list">
                  <li><ArrowIcon /> Emailre 24 órán belül válaszolok</li>
                  <li><ArrowIcon /> Az első konzultáció ingyenes és kötelezettségmentes</li>
                  <li><ArrowIcon /> Az audit után mindig kapsz árajánlatot a javításra — te döntesz</li>
                  <li><ArrowIcon /> Általában 1 hetes várólistával számolj</li>
                </ul>
              </motion.div>
        ...
        <div className="wrap" style={{ marginTop: 'clamp(44px, 6vw, 80px)' }}>
          ...
          {submitted ? (
            ...
            >
              <div className="field">
                <label htmlFor="cf-name">Név <span aria-hidden="true">*</span></label>
                <input
                  id="cf-name" name="name" type="text" autoComplete="name" required
                  maxLength={100} value={fields.name} onChange={handleChange}
                  aria-describedby={errors.name ? 'cf-name-err' : undefined}
                  aria-invalid={!!errors.name}
                />
                {errors.name && (
                  <span className="field-error" id="cf-name-err" aria-live="polite">{errors.name}</span>
                )}
              </div>
              ...
              <div className="field">
                ...
                <input
                  ...
                  maxLength={200} value={fields.email} onChange={handleChange}
                  aria-describedby={errors.email ? 'cf-email-err' : undefined}
                  aria-invalid={!!errors.email}
                />
                {errors.email && (
                  <span className="field-error" id="cf-email-err" aria-live="polite">{errors.email}</span>
                )}
              </div>
              ...
              <div className="field">
                ...
                <input
                  ...
                  maxLength={30} value={fields.phone} onChange={handleChange}
                />
              </div>

              <div className="field">
                <label htmlFor="cf-service">Miben segíthetünk?</label>
                <select id="cf-service" name="service" value={fields.service} onChange={handleChange}>
              ...
              <div className="field">
                <label htmlFor="cf-message">Üzenet <span aria-hidden="true">*</span></label>
                <textarea
                  id="cf-message" name="message" rows={5} required maxLength={2000}
                  value={fields.message} onChange={handleChange}
                  aria-describedby={errors.message ? 'cf-message-err' : undefined}
                  aria-invalid={!!errors.message}
                />
                {errors.message && (
                  <span className="field-error" id="cf-message-err" aria-live="polite">{errors.message}</span>
                )}
              </div>

              {serverError && (
                <div className="form-error-msg" role="alert">{serverError}</div>
              )}

              <button
                id="cf-submit" type="submit" className="btn btn-primary btn-lg"
